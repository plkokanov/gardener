...
| VPARecommenderHistoryFromPrometheus      | `false` | `Alpha` | `1.114` |         |
...
| VPARecommenderHistoryFromPrometheus      | `gardenlet`, `gardener-operator`   | Enables the vpa-recommender to use historical data from prometheus instead of relying on checkpoints after restarts. If monitoring is disabled a special prometheus instance that only scrapes the CAdvisor for metrics will be created in the shoot's control plane namespace, seed's garden namespace and garden runtime cluster's garden namespace. These prometheus instances will be used to provide history to the vpa-recommenders running in the same namespaces. |