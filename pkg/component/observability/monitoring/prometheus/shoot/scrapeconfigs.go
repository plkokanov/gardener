...
func CentralScrapeConfigs(namespace, clusterCASecretName string, isWorkerless, usePrometheusForVPARecommenderMetricsHistory bool) []*monitoringv1alpha1.ScrapeConfig {
	...
	if !isWorkerless {
		out = append(out,
			...
			centralCAdvisorScrapeConfig(clusterCASecretName, usePrometheusForVPARecommenderMetricsHistory),
		)
	}
...
func centralCAdvisorScrapeConfig(clusterCASecretName string, usePrometheusForVPARecommenderMetricsHistory bool) *monitoringv1alpha1.ScrapeConfig {
	metricsKeepSourceLabel := []monitoringv1.LabelName{"namespace"}
	// systemd containers don't have namespaces
	metricsKeepRegex := `(^$|^kube-system$)`

	if usePrometheusForVPARecommenderMetricsHistory {
		// Keep all metrics in kube-system namespace, and only metrics relevant to vpa-recommender from all other namespaces.
		metricsKeepSourceLabel = []monitoringv1.LabelName{"namespace", "__name__"}
		metricsKeepRegex = `(^(|kube-system);.*|.*;(container_memory_working_set_bytes|container_cpu_usage_seconds_total)$)`
	}
	...
	return &monitoringv1alpha1.ScrapeConfig{
		...
		Spec: monitoringv1alpha1.ScrapeConfigSpec{
			...
			MetricRelabelConfigs: []monitoringv1.RelabelConfig{
				...
				{
					SourceLabels: metricsKeepSourceLabel,
					Action:       "keep",
					// systemd containers don't have namespaces
					Regex: metricsKeepRegex,
				},
				{
